...
class QwenImagePlusPipeline(BasePipeline):
    ...
    def __init__(self, config):
        ...
        tokenizer = transformers.Qwen2Tokenizer.from_pretrained('Qwen/Qwen-Image-Edit-2509/tokenizer', local_files_only=True)
        processor = transformers.Qwen2VLProcessor.from_pretrained('Qwen/Qwen-Image-Edit-2509/processor', local_files_only=True)
        ...
        text_encoder_config = transformers.Qwen2_5_VLConfig.from_pretrained('Qwen/Qwen-Image-Edit-2509/text_encoder', local_files_only=True)
        ...
        with open('Qwen/Qwen-Image-Edit-2509/vae/config.json') as f:
        ...
        self.diffusers_pipeline = diffusers.QwenImageEditPlusPipeline(
            processor=processor,
            scheduler=None,
            vae=vae,
    ...
    def load_diffusion_model(self):
        ...
        with open('Qwen/Qwen-Image-Edit-2509/transformer/config.json') as f:
    ...
    def get_call_vae_fn(self, vae):
        def fn(*args):
            image = args[0]  # main image latents to denoise
    ...
    def _get_qwen_prompt_embeds(
        ...
        if control_files is None:
            ...
            if not isinstance(control_files, (list, tuple)):
    ...
    def prepare_inputs(self, inputs, timestep_quantile=None):
        ...
        if 'control_latents' in inputs:
            ...
            if c_cl != channels or f_cl != num_frames:
                raise ValueError(f'control_latents channel/frame mismatch: {(c_cl, f_cl)} vs {(channels, num_frames)}')
            ...
            control_latents_packed = torch.cat(packed_controls, dim=1)
            img_seq_len = torch.tensor(x_t.shape[1], device=x_t.device).repeat((bs,))
            extra = (img_seq_len,)
            x_t = torch.cat([x_t, control_latents_packed], dim=1)
        else:
            extra = tuple()
